...
const MovieDetails = () => {
    ...
    const { title, release_date, poster_path, overview, genres, vote_average } = movie

    return (
        <div className="container">
            <Link to={backLinkHref} className="inline-flex gap-2 items-center p-2"><FiArrowLeftCircle className='inline' />Go Back</Link>
            <div className="flex gap-8 flex-wrap py-2 items-center justify-center">
                ...
                <div className="flex flex-col gap-3 max-w-[75%] py-4">
                    ...
                    <div className="flex gap-x-[10%] gap-y-1 flex-wrap">
                        ...
                        {vote_average && <p>User Score: {Math.round((vote_average / 10) * 100)}%</p>}
                    </div>
                    <div>
                        <h3>Overview</h3>
                        <p>{overview}</p>
